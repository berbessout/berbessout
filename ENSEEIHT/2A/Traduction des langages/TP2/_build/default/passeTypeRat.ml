...
let rec analyse_type_instruction i =
  match i with 
    ...
    |AstTds.Conditionnelle (nc, tast, east) -> let (e,t) = analyse_type_expression nc in
                                                  if est_compatible t Bool then AstType.Conditionnelle(e, analyse_type_bloc tast, analyse_type_bloc east)
    ...
    |AstTds.TantQue(c, b) -> (let (nc,t) = analyse_type_expression c in 
                                            if Type.est_compatible t Bool then AstType.TantQue(nc, analyse_type_bloc b) else raise (TypeInattendu(t, Bool)))
...
and analyse_type_bloc bloc =
  ...
  let nli = List.map (analyse_type_instruction) bloc in
  ...
  let analyse_type_fonction (AstTds.Fonction(t,info,lp,li)) =
    ...
    AstType.Fonction(info, linfo, analyse_type_bloc li)
