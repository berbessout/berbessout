...
let%test_unit "testRecursiviteVariable" = 
  ...
  let%test_unit "testP1" =
    let _ = compiler (pathFichiersRat^"testP1.rat") in ()

let%test_unit "testP2" = 
  let _ = compiler (pathFichiersRat^"testP2.rat") in ()

let%test_unit "testP3E" = 
  try 
    let _ = compiler (pathFichiersRat^"testP3E.rat")
    in raise ErreurNonDetectee
  with
  | IdentifiantNonDeclare("y") -> ()
...
try
  let _ = compiler (pathFichiersRat^"testFor3.rat") 
in raise ErreurNonDetectee
with
| MauvaiseUtilisationIdentifiant("i") -> ()
