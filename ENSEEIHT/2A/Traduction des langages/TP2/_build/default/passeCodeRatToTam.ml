...
let rec analyse_instruction i = 
  match i with 
    ...
    |AstPlacement.Goto(i) -> (match info_ast_to_info i with
                                |InfoEthiq(n,contexte) -> jump n^"goto"^contexte
...
and analyser_bloc (l, taille) = 
  ...
  let analyser_code_fonction (AstPlacement.Fonction(info,_,l)) =
...
let rec analyser (AstPlacement.Programme (fonctions,bloc)) =
  getEntete() ^
  (List.fold_left (^) "" (List.map (analyser_code_fonction) fonctions)) 
  ^ "Main \n" ^
  analyser_bloc bloc ^
  halt