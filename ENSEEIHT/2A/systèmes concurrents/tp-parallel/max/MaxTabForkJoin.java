    ...
    static class PartialMax extends RecursiveTask<Integer> {
        ...
        public Integer compute() {
            ...
            if (this.end - this.start > threshold) {
                int midle = (this.end + this.start)/2;
                PartialMax max1 = new PartialMax(array, start, midle, threshold);
                PartialMax max2 = new PartialMax(array, midle, end, threshold);
                max1.fork();
                max2.fork();

                int result = Math.max(max1.join(), max2.join());
                return result;
            } else {
