...
struct
  ...
  let get_branche code arbre =
    match arbre with 
    | Lettre (f) -> failwith "sur une feuille"
    | Noeud (lb) -> 
      let suivant = List.assoc_opt code lb
      in match suivant with
      | None -> None
      | _ -> suivant
  ...
  let rec dechiffrer  code arbre =
      let rec aux code arbre =
        match arbre with
        |Lettre(a) -> (a, code)
        |Noeud(lb) -> match code with
                      | [] -> raise CodeNonValide
                      | t::q -> let suivant = get_branche t arbre in
                                match suivant with
                                | None -> raise CodeNonValide
                                | Some a -> aux q a
      in if code = [] then []
      else 
        let (symbole, suite) = aux code arbre in 
        symbole::(dechiffrer suite arbre)
...
let%test _ = dechiffrer [1;2;3;2;1;2] arbre1 = ['b';'a';'c']
  ...
  let  rec arbre_to_liste  arbre = 
        match arbre with 
        | Lettre (a) -> [(a, [])]
        | Noeud (lb) -> parcours_b lb
  
  and parcours_b lb =
    match lb with
    |[] -> []
    |(cle, suivant)::q ->(List.map (fun (a, b) -> (a, cle::b)) (arbre_to_liste suivant)) @ (parcours_b q)
  ...
  let chiffrer  texte arbre = 
    let codage = arbre_to_liste arbre in
    let rec aux texte = 
      match texte with
      | [] -> []
      | t::q -> let codee = List.assoc_opt t codage in
                match codee with
                | None -> raise DonneeNonValide
                | Some code -> code @ (aux q)
    in aux texte
...
struct
   ...
   *)
  let rec dechiffrer code arbre = 
    let codelist = DRCode.decompose code in
    DRDonnee.recompose (Arbre.dechiffrer codelist arbre)
  ...
  let chiffrer donnee arbre =
    let donneelist = DRDonnee.decompose donnee in
    DRCode.recompose (Arbre.chiffrer donneelist arbre)
