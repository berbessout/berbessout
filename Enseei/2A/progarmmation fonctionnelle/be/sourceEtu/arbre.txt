...
module Arbre (B : Base) = 
  struct
    ...
    let rec conforme arbre =
      match arbre with
      | Vide -> true
      | Noeud (b, lb) -> (List.length lb = B.base) && (conforme_l lb)
    
    and conforme_l lb = 
      match lb with 
      | [] -> true
      | t::q -> (conforme t) && (conforme_l q)
    ...
    let rec appartient nbr arbre = 
      match arbre with
      |Vide -> false
      |Noeud(b, lb) -> (match nbr with
                      | [] -> b
                      | t::q -> let suivant = get t lb in
                                appartient q suivant)
    
    (* ajouter : int list -> arbre -> arbre *)
    (* Ajoute un entier, décomposé en base B, à un ensemble d'entier codé avec un arbre *)
    (* L'ensemble de départ et l'ensemble d'arrivée sont conformes à la base *)
    let rec ajouter nbr arbre = 
      match nbr with
      | [] -> (match arbre with
              | Vide -> Noeud (true, Liste.l_taille B.base Vide)
              | Noeud(b, lb) -> Noeud (true, lb))
      |t::q -> (match arbre with 
              | Vide -> Noeud (false, Liste.set t (Liste.l_taille B.base Vide) (ajouter q Vide))
              | Noeud (b, lb) -> Noeud (b, Liste.set t lb (ajouter q (Liste.get t lb))))
    ...
    let rec retirer nbr arbre =
      match arbre with
      | Vide -> Vide
      | Noeud(b, lb) -> (match nbr with
                        | [] -> (if (lb = Liste.l_taille B.base Vide) then Vide
                                else Noeud (false, lb))
                        | t::q -> (let nettoyee = Liste.set t lb (retirer q (Liste.get t lb)) in
                                  if (nettoyee = Liste.l_taille B.base Vide && not b) then Vide
                                  else Noeud (b, nettoyee)))
  end
...
module TestArbre2 = struct
  ...
  (* retirer *)
  let%test _ =  retirer [ 1; 0; 0 ] (*4*) arbre_0_1_4_7_14_36_base2 = arbre_0_1_7_14_36_base2
  let%test _ =  retirer [ 1; 0; 0; 1; 0; 0 ] (*36*) arbre_0_1_7_14_36_base2 = arbre_0_1_7_14_base2
...
module TestArbre5 = struct
  ...
  let%test _ = ajouter [ 1; 2; 1 ] (*36*) arbre_0_1_7_14_base5 = arbre_0_1_7_14_36_base5
  let%test _ = ajouter [ 4 ] (*4*) arbre_0_1_7_14_36_base5 = arbre_0_1_4_7_14_36_base5


  (* retirer *)
  let%test _ = retirer [ 4 ] (*4*) arbre_0_1_4_7_14_36_base5 = arbre_0_1_7_14_36_base5
  let%test _ =  retirer [ 1; 2; 1 ] (*36*) arbre_0_1_7_14_36_base5 = arbre_0_1_7_14_base5
