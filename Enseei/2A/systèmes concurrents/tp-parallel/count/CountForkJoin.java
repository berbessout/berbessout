    ...
    static class PartialCount extends RecursiveTask<Integer> {
        ...
        public Integer compute() {
            if (this.end - this.start > threshold) {
                ...
                PartialCount max1 = new PartialCount(array, start, midle, threshold);
                PartialCount max2 = new PartialCount(array, midle, end, threshold);
                max1.fork();
                max2.fork();

                int result = max1.join() + max2.join();
                return result;
            } else {
